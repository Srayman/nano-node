...
namespace websocket
{
	...
	enum class topic
	{
		...
		stopped_election,
		/** Change in quorum for an election **/
		quorum_change,
		/** A vote message **/
		vote,
	...
	{
	public:
		message block_confirmed (std::shared_ptr<nano::block> block_a, nano::account const & account_a, nano::amount const & amount_a, std::string subtype, bool include_block, nano::election_status const & election_status_a, std::vector<nano::vote_with_weight_info> const & election_votes_a, nano::websocket::confirmation_options const & options_a);
		message stopped_election (nano::block_hash const & hash_a);
		message quorum_change (nano::root const & root_a, nano::election_status const & election_status_a, std::vector<nano::vote_with_weight_info> const & election_votes_a);
	...
	public:
		...
		void broadcast_confirmation (std::shared_ptr<nano::block> block_a, nano::account const & account_a, nano::amount const & amount_a, std::string subtype, nano::election_status const & election_status_a, std::vector<nano::vote_with_weight_info> const & election_votes_a);

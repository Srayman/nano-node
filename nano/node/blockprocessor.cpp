...
void nano::block_processor::process_batch (nano::unique_lock<std::mutex> & lock_a)
{
	...
	while (have_blocks_ready () && (!deadline_reached () || !processor_batch_reached ()) && !awaiting_write && !store_batch_reached ())
	{
		...
		else
		{
			...
			if (forced.empty ())
			{
				std::tie (info, watch_work) = blocks.top ();
				blocks.pop ();
			...
			if (force)
			{
				...
				if (successor != nullptr && successor->hash () != hash)
				{
					...
					for (auto & i : rollback_list)
					{
						...
						{
							node.logger.always_log (boost::str (boost::format ("Erase block %1% with successor %2%") % i->hash ().to_string () % successor->hash ().to_string ()));
							node.active.erase (*i);
						}
					}
				}
